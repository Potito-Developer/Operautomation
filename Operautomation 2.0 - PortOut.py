...
def initIExplorer(savepath = ""):
    global browser
    browser = webdriver.Ie("plugin\\IEDriverServer.exe")
...
def readExcelCell(information):
    global registerT
    pathToFile = information.split(",")[0]
    cellCoordinates = information.split(",")[1]
    if not "\\" in pathToFile:
        pathToFile = cycleIDs[pathToFile][1]
    else:
        pathToFile = workspace + "\\" + pathToFile[1:]
    ...
    df = xl.parse(xl.sheet_names[0])
    try:
        registerT = df.iloc[int(cellCoordinates.split("-")[0]), int(cellCoordinates.split("-")[1])]
    except:
        registerT = ""
...
for line in lines:
    ...
    if instruction == "prepareWorkspace":
        ...
        selectFromArray(int(information))
